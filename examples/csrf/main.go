...
func main() {
	...
	log.Println("  GET  /           - Overview and documentation")
	log.Println("  GET  /form       - HTML form with CSRF protection")
	log.Println("  POST /submit     - Form submission handler")
	log.Println("  GET  /api        - AJAX/Fetch API demo")
	log.Println("  POST /api/update - API endpoint protected by CSRF")
	log.Println("")
	log.Println("Try these curl commands:")
	log.Println("  # Without CSRF token (will fail with 403):")
	log.Println(`  curl -X POST http://localhost:8080/submit -d "message=hello"`)
	log.Println("")
	log.Println("  # First get a CSRF token (save cookie jar):")
	log.Println(`  curl -s http://localhost:8080/form -c cookies.txt > /dev/null`)
	log.Println("")
	log.Println("  # Then submit with token from cookie jar:")
	log.Println(`  CSRF_TOKEN=$(grep csrf_token cookies.txt | tail -1 | awk '{print $7}')`)
	log.Println(`  curl -X POST http://localhost:8080/submit -b cookies.txt -d "csrf_token=$CSRF_TOKEN" -d "message=hello"`)
	log.Fatal(app.Start())
}
...
curl -X POST http://localhost:8080/submit \
  ...
  -d "message=hello"</pre>
    ...
    <ul>
        ...
        <li><code>query:csrf_token</code> - For query parameters (not recommended)</li>
    </ul>
</body>
</html>`
