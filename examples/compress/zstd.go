...
// Example: Zstandard (zstd) Compression
//
// This example shows how to add zstd compression using the
...
func main() {
	...
	// Use compression with zstd support
...
<p>Try: curl -H 'Accept-Encoding: zstd' http://localhost:8080/ | zstd -d</p>
</body>
</html>`))
		return err
	}))

	app.Logger().Info("Starting server with zstd + gzip + deflate support")
