...
- **Metrics Middleware (pluggable interface)** - Request metrics collection with built-in Prometheus-compatible `/metrics` endpoint (zero dependencies). Pluggable interface allows users to bring their own metrics library (e.g., Prometheus client, OpenTelemetry).
- **JWT Middleware (pluggable interface)** - Pluggable JWT validation interface, allowing users to bring their own JWT library (e.g., `golang-jwt/jwt`). Similar pattern to existing pluggable features like WebSocket and HTTP/3.
