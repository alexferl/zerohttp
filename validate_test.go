...
func TestIsZeroValue_Interface(t *testing.T) {
	// Test interface type in isZeroValue (should return false as default)
	var iface any = "hello"
	v := reflect.ValueOf(&iface).Elem() // Get the interface value
	if isZeroValue(v) {
	...
	// nil interface
	var nilIface any
	nilV := reflect.ValueOf(&nilIface).Elem()
	if !isZeroValue(nilV) {
...
func TestIsZeroValue_Channel(t *testing.T) {
	// Test isZeroValue with channel (should hit default case and return false)
	ch := make(chan int)
	v := reflect.ValueOf(ch)
	if isZeroValue(v) {
		t.Error("expected non-nil channel to not be zero")
	}

	// Note: nil channels also hit default case (returns false) since
	// isZeroValue doesn't have explicit case for reflect.Chan
}

func TestIsZeroValue_DefaultCase(t *testing.T) {
	// Test with function type (should hit default case and return false)
	fn := func() {}
	v := reflect.ValueOf(fn)
	if isZeroValue(v) {
		t.Error("expected non-nil function to not be zero (default case)")
	}

	// Test with unsafe.Pointer (also hits default case)
	// We can't easily test this without importing unsafe, but function covers default case
}
