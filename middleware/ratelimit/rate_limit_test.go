...
func TestRateLimit_DefaultConfigFallbacks(t *testing.T) {
	tests := []struct {
		...
		{"rate", func() func(http.Handler) http.Handler { return New(Config{Rate: 0}) }},
		{"window", func() func(http.Handler) http.Handler { return New(Config{Window: 0}) }},
		{"algorithm", func() func(http.Handler) http.Handler { return New(Config{Algorithm: ""}) }},
		{"key extractor", func() func(http.Handler) http.Handler { return New(Config{KeyExtractor: nil}) }},
	}
...
func TestRateLimitStatusCodeAndMessageDefaults(t *testing.T) {
	m := New(Config{StatusCode: 0, Rate: 1, Window: time.Minute})
...
func TestRateLimitMessageDefaults(t *testing.T) {
	m := New(Config{Message: "", Rate: 1, Window: time.Minute})
...
func TestJWTSubjectKeyExtractor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			claims:   jwtauth.HS256Claims{"sub": "user123"},
			expected: "user123",
		},
		{
			name:     "missing subject claim",
			claims:   jwtauth.HS256Claims{"iss": "my-issuer"},
			expected: "",
		},
		{
			name:     "empty claims",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), jwtauth.ClaimsContextKey, tt.claims)
			req := zhtest.NewRequest(http.MethodGet, "/test").Build().WithContext(ctx)
