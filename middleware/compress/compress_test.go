...
func TestCompressConfigExplicitEmptyValues(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			config:            Config{Algorithms: []Algorithm{}},
		...
		},
		{
			...
			config:            Config{ExcludedPaths: []string{}},
...
func TestCompressConfigDefaultsVsOverrides(t *testing.T) {
	...
	t.Run("overrides work when explicitly set", func(t *testing.T) {
		mw := New(Config{
			...
			Algorithms: []Algorithm{Gzip},       // Gzip only
		})
...
// TestCompressionProvider tests the pluggable Provider interface
func TestCompressionProvider(t *testing.T) {
	t.Run("custom encoder is registered and used", func(t *testing.T) {
		...
		provider := &testProvider{encoders: map[string]Encoder{
			"nop": nopEncoder,
		}}

		mw := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{"nop", Gzip},
	...
	t.Run("custom encoder with level parameter", func(t *testing.T) {
		...
		mw := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{"testlevel"},
			Level:      9,
			Providers:  []Provider{provider},
		})
	...
	t.Run("multiple custom encoders from provider", func(t *testing.T) {
		...
		mw := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{"custom1", "custom2"},
			Providers:  []Provider{provider},
		})
	...
	t.Run("custom encoder alongside built-in", func(t *testing.T) {
		...
		mw := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{Gzip, "custom"},
			Providers:  []Provider{provider},
		})
	...
	t.Run("multiple providers", func(t *testing.T) {
		...
		provider1 := &testProvider{encoders: map[string]Encoder{
			"brotli": encoder1,
		}}
		provider2 := &testProvider{encoders: map[string]Encoder{
			"zstd": encoder2,
		}}

		mw := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{"brotli", "zstd"},
	...
	t.Run("algorithms order determines precedence with providers", func(t *testing.T) {
		...
		brotliProvider := &testProvider{encoders: map[string]Encoder{
			"br": brotliEncoder,
		}}
		zstdProvider := &testProvider{encoders: map[string]Encoder{
			"zstd": zstdEncoder,
		}}
		...
		mw := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{"zstd", "br"},
			Providers:  []Provider{brotliProvider, zstdProvider},
		})
		...
		mw2 := New(Config{
			Types:      []string{"text/plain"},
			Algorithms: []Algorithm{"br", "zstd"},
			Providers:  []Provider{zstdProvider, brotliProvider},
		})
...
// testEncoder is a simple test implementation of Encoder
...
func (p *testProvider) GetEncoder(encoding string) Encoder {
