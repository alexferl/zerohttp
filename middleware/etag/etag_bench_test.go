...
// BenchmarkETag_GenerateETag measures New generation with different algorithms
func BenchmarkETag_GenerateETag(b *testing.B) {
	...
	for _, size := range sizes {
		...
		b.Run(fmt.Sprintf("FNV_Weak_%dB", size), func(b *testing.B) {
			weak := true
			ew := &etagResponseWriter{
				config:         Config{Algorithm: FNV, Weak: &weak},
				ResponseBuffer: &rwutil.ResponseBuffer{Buf: *bytes.NewBuffer(data)},
			}
	...
	for _, algo := range algorithms {
		for _, s := range sizes {
			b.Run(algo.name+"/"+s.name, func(b *testing.B) {
				handler := New(Config{Algorithm: algo.algo})(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
// BenchmarkETag_Match compares different New matching scenarios
func BenchmarkETag_Match(b *testing.B) {
	...
	for _, tc := range cases {
		b.Run(tc.name, func(b *testing.B) {
			...
			for b.Loop() {
				_ = Matches(tc.ifNoneMatch, tc.etag)
			}
		})
	}
}
...
// BenchmarkETag_FileETag measures file New generation
func BenchmarkETag_FileETag(b *testing.B) {
	b.Run("Weak", func(b *testing.B) {
		...
		for b.Loop() {
			_ = GenerateFromFile(1709999999, 1024*1024*100, true)
		}
	})
	...
	b.Run("Strong", func(b *testing.B) {
		...
		for b.Loop() {
			_ = GenerateFromFile(1709999999, 1024*1024*100, false)
		}
	})
}

// BenchmarkETag_ParseETag measures New parsing
...
// BenchmarkETag_Concurrent measures concurrent New generation
