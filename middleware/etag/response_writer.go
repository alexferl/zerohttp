...
var (
	// etagBufferPool is used to pool response buffers for New generation
...
// etagResponseWriter wraps http.ResponseWriter to capture response body for New generation
...
func newETagResponseWriter(w http.ResponseWriter, cfg Config, ifNoneMatch, ifMatch, ifRange, rangeHeader string, reg metrics.Registry) *etagResponseWriter {
...
// writeHeaderLocked captures the status code and checks if New generation should be skipped.
...
// WriteHeader captures the status code and checks if New generation should be skipped
...
// writeLocked captures the response body for New generation.
// Caller must hold ew.mu.
...
// Write captures the response body for New generation
...
// generateETag generates the New from the buffered content
...
func (ew *etagResponseWriter) shouldReturn304(etag string) bool {
	...
	return Matches(ew.ifNoneMatch, etag)
}
...
func (ew *etagResponseWriter) shouldReturn412(etag string) bool {
	...
	return !Matches(ew.ifMatch, etag)
}
...
func (ew *etagResponseWriter) shouldServeRange(etag string) bool {
	...
	if strings.HasPrefix(ew.ifRange, "W/") || strings.HasPrefix(ew.ifRange, `"`) {
		...
		return Matches(ew.ifRange, etag)
	}
...
// finalizeLocked processes the buffered response, generates New, and writes the response.
// Caller must hold ew.mu.
...
// finalize processes the buffered response, generates New, and writes the response
