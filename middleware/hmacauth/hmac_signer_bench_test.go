...
func BenchmarkHMACSigner_GenerateSignature(b *testing.B) {
	tests := []struct {
		...
		{"SHA256", SHA256, "test-secret-that-is-32-bytes-long!!"},
		{"SHA384", SHA384, "test-secret-that-is-48-bytes-long-for-sha384-algo!"},
		{"SHA512", SHA512, "test-secret-that-is-64-bytes-long-for-the-sha512-algorithm-use!!"},
	}

	for _, tt := range tests {
		b.Run(tt.name, func(b *testing.B) {
			signer := NewSignerWithAlgorithm("test-key", tt.secret, tt.algo)
			req := httptest.NewRequest(http.MethodGet, "/api/test", nil)
			req.Host = "example.com"
