...
func BenchmarkRequestLogger_FieldConfiguration(b *testing.B) {
	fieldConfigs := []struct {
		...
		{"Minimal", []LogField{FieldMethod, FieldPath, FieldStatus}},
		{"Default", DefaultConfig.Fields},
		{"AllFields", []LogField{
			FieldMethod, FieldURI, FieldPath, FieldHost,
			FieldProtocol, FieldReferer, FieldUserAgent,
			FieldStatus, FieldDurationNS, FieldDurationHuman,
			FieldRemoteAddr, FieldClientIP, FieldRequestID,
		}},
	}
...
// BenchmarkRequestLogger_LogRequest benchmarks the Log function directly
func BenchmarkRequestLogger_LogRequest(b *testing.B) {
	cfg := Config{
		Fields: []LogField{
			FieldMethod, FieldURI, FieldPath,
			FieldStatus, FieldDurationNS,
		},
	}
	...
	for b.Loop() {
		Log(logger, cfg, fieldMap, req, http.StatusOK, 5*time.Millisecond, "", "")
	}
}
